...
class SHA(Dataset):
    ...
    def __init__(self, opt, split, transform=None, flip=False):

        data_root = os.path.join(opt.data_dir, 'ShanghaiTech/Part_A')
        ...
        self.trans_default = transforms.Compose([transforms.ToTensor()])
    ...
    def pad_image(self, image):
        ...
        if pad_width>0 or pad_height>0:
            # 定义填充转换
            padding_transform = transforms.Pad(padding=(0, 0, pad_width, pad_height), fill=0)
            # 进行填充操作
    ...
    def __getitem__(self, index):
        ...
        img = self.trans_default(img) # torch.Tensor(img)
